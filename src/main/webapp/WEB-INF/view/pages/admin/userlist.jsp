        ...
        <div class="search mb-3">
            <form action="/user/list" method="get">
                <div class="input-group">
                    <input type="text" class="form-control" placeholder="유저 이름 검색" name="name">
                    ...
                    <tbody>
                        <c:forEach var="user" items="${userList}">
                            <tr>
                                ...
                                <td class="text-center">${user.gender == 1 ? '남성' : '여성'}</td> <!-- 성별을 한글로 표시합니다. -->
                                <td class="text-center">${user.email}</td>
                                <td class="text-center">${user.phone}</td>
                                <td class="text-center">${user.type == 1 ? '일반계정' : '관리자'}</td> <!-- 계정 유형을 한글로 표시합니다. -->
                                <td class="text-center">${user.point}</td>
                                <td class="text-center"><button type="button" class="btn btn-warning">자세히보기</button></td>
                            </tr>
                        </c:forEach>
                    </tbody>
                </table>
